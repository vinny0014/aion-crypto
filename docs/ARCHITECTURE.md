...
```
Browser ──> Next.js 14 (App Router, SSR/ISR, revalidate 60s)
                │  server-side fetch
                ▼
...
Admin UI, Discovery/Content/Verification/Image agents on top of Commander, Monitor Recovery loop, Alembic migrations for PostgreSQL, newsletter double opt-in delivery, GA4 wiring.
