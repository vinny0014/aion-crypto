...
migration removes inherited public grants and enables RLS as defence in depth.
It deliberately does not create permissive policies.
...
def upgrade() -> None:
    ...
    # re-grant access unless a reviewed browser-side Supabase use case is added.
    for table in TABLES:
        op.execute(f"ALTER TABLE public.{table} ENABLE ROW LEVEL SECURITY")

    # Supabase creates these API roles, while the vanilla PostgreSQL service in
    # CI does not. Resolve roles at runtime without creating platform roles.
...
def downgrade() -> None:
    ...
    # A reviewed forward corrective migration must explicitly restore any
    # browser-side access with narrowly scoped grants and RLS policies.
